...
def test_local_artifact_family_analytics_builder_deduplicates_repeated_candidate_runs(tmp_path):
    plan_result = PlanWorkflow(
        hypothesis_planner=StaticHypothesisPlanner(),
        blueprint_builder=StaticBlueprintBuilder(),
    ).run(build_sample_research_agenda())
    ...
    candidate_stage_records = []
    run_ids = [
        "simulate.quality_deterioration.001",
        "simulate.quality_deterioration.002",
    ]
    ...
    assert analytics.family_stats[0].total_candidates == 4
    assert analytics.family_stats[0].simulation_candidate_count == 4
    assert analytics.family_stats[0].stage_a_evaluation_count == 4
    assert analytics.family_stats[0].last_run_id == "simulate.quality_deterioration.002"
