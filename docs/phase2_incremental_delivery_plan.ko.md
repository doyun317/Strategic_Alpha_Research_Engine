...
- `StaticValidationIssue`, `StaticValidationReport` 도메인 추가
- metadata catalog 기반 operator/field 검증 추가
- balanced parentheses / allowed operator / allowed field / basic arity 검사
- synthesizer와 critic 사이가 아니라 critic 이전 독립 단계로 validator 배치
- validator 단위 테스트와 workflow 연동 테스트 추가
...
1. validation domain 모델 추가
2. validator service 추가
3. workflow 또는 CLI 연결
4. 테스트 추가
5. 문서 상태 업데이트
...
- `pytest` 통과
- invalid operator / invalid field / unbalanced parentheses 케이스 차단 확인
- valid candidate는 validator를 통과하고 기존 critic 흐름으로 이어짐
- validator가 metadata catalog를 실제로 참조함
