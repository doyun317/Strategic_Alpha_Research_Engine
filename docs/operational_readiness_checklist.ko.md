...
이 문서는 현재 구현된 Phase 1~5 범위를 실제로 다시 실행하면서
...
3. artifact, state, lineage가 실제로 일관적인지 확인한다.
...
- [ ] 실제 외부 API 연동은 `simulate --brain-provider worldquant` 로 별도 검증해야 한다는 점을 이해하고 있다.
